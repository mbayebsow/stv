...
const ChannelList = ({ data, leadingText }) => {
  ...
  const { channels, channelActiveIndex, groupActiveIndex, setChannels, setChannelActiveIndex } = useGlobalStore()
  ...
  return (
    ...
    >
      {channels.map((channel, i) => (
        <div
          key={i}
          onClick={() => setChannelActiveIndex(i)}
          className={cn(
          ...
          style={{
            ...
            transform: `translateY(${determinePlacement(i) - windowSize.current[1]/9}px)`,
          }}
        >
          <div
            className={`${
                channelActiveIndex === i
                  ? "bg-white/30 shadow-xl scale-105"
